...
| `/collaboration-opportunities/` | `collaboration-opportunities` | Keeps the three collaboration tracks, the credential blurbs and the video row. The Divi contact form is **dropped, not ported** — see "The two contact forms" under content that will not carry over. The built route has zero `<form>` elements; the collaboration ask is a `mailto:` and the office phone number. |
...
| `/contact-us/` | `contact-us` | Gains a real H1, a working `mailto:`, a `tel:` link, and **a real booking form** posting to `app/api/contact/route.ts`. **AMENDED 2026-08-07:** this row said "No contact form" and that stopped being true when the endpoint was built. The source page has none either (`_source/pages/contact-us.md`, "Form fields (verbatim)": zero `<form>`, `<input>`, `<textarea>`, `<label>`, `<button>` in MAIN), so this is not a parity loss. The only `<form>` the built route carries is the Mailchimp newsletter signup at No. 04, which posts to a real third-party endpoint. See "The two contact forms" below. |
...
**AMENDED 2026-08-07.** A real speaker one-sheet now exists at `/speaker-one-sheet/`, with a PDF of the same page generated from it by `scripts/build-one-sheet.mjs`. Open item #3 is closed. The removed link's destination, `AG-MEDIA-KIT-3.zip`, was also recovered before the staging host went away: it held nine professional photographs and its contents are inventoried in `_source/media-kit/PROVENANCE.md`.
...
### The one supplied mark that ships but is never placed

| Processed, not placed | Why |
|---|---|
| `Damian Mason Logos/BOASG-BRAND-LOGO-FINAL-WHITE.jpg` → `public/img/brand/boasg-white-flat.jpg`, exposed as `brandAssetsExtra.boasgWhiteFlat` | Not dropped, and nothing is lost. It is the same 800 by 800 artwork as the transparent `boasg-white.png` badge that `/boasg/` already renders, differing only in carrying an opaque white ground instead of alpha: compared greyscale at 400px, mean absolute difference 0.28 of 255, with the largest deviations along the jpeg's own edge ringing. Every route that wants the badge wants it over a photograph or a dark panel, which is the transparent file's job, so the flat copy has no placement. It is processed and kept rather than skipped because it is a supplied client asset and the opaque version is the one to hand somebody for print or an email signature. |

### The site icon

| Ships as | From |
|---|---|
| `app/favicon.ico` (16/32/48 frames), `app/icon.png` (512), `app/apple-icon.png` (180) | All three are the client's own DM monogram, navy on white, cut from `_source/media/cropped-favicon.jpg`, which is the favicon the old site served. `scripts/normalize-assets.mjs:142` transcodes that source into `public/img/brand/dm-monogram.jpg`. **`app/icon.png` is the master** and is committed as artwork: the square crop was made by eye and is not re-derived, because re-cutting it from the JPEG would change the mark silently. The other two are pure resizes of that master and are built by **`scripts/build-app-icons.mjs`** (`npm run build:icons`, or `npm run check:icons` to assert the committed files still match the master and fail if they have drifted). Both currently reproduce byte for byte. That script replaces a `_ico-tmp.mjs` that was the only route back to the shipped `.ico` while carrying a filename every later pass was entitled to delete. `brandAssetsExtra.dmMonogram` points at the source the icons were cut from, not at an image any route renders; its comment used to claim it was the favicon itself, which it never was. Recorded here because Phase 0 shipped the `create-next-app` scaffold `favicon.ico`, Vercel's white triangle in a black circle, on all 19 routes, so every browser tab, bookmark and history entry for damianmason.com carried the framework vendor's logo. Next.js emits `<link rel="icon">` for the first two and `<link rel="apple-touch-icon">` for the third on every route automatically; there is no hand-written link tag and no web manifest. Verified in round 4: all three return 200 and all three `<link>` tags are present on every one of the 19 routes, and no reference to the scaffold mark survives anywhere in `app/`, `components/`, `content/`, `lib/`, `src/` or `scripts/`. |
...
| **The two contact forms** — the Divi `et_pb_contact_form_0` on `/collaboration-opportunities/` (Email → Full Name → Phone → Message, "Send Message"), and the contact form this manifest itself previously promised as new on `/contact-us/` | **Corrected disposition, recorded because this file had it wrong.** Earlier revisions of the Route map said `/collaboration-opportunities/` "keeps the contact form (form needs a new backend)" and that `/contact-us/` "gains (new) a contact form." Neither is what shipped, and one of them was never portable in the first place. The Divi form POSTs to the page's own URL with a `_wpnonce-et-pb-contact-form-submitted-0` hidden field — a WordPress-only mechanism with no reusable endpoint, already flagged as "needs replacing wholesale" in `_source/pages/collaboration-opportunities.md`. Replacing it wholesale needs a server to replace it with. **This build had no backend when that was written**, so a form would have been a set of inputs with nowhere to POST: it would look like a working inquiry channel, collect a meeting planner's date and budget, and drop them. That is strictly worse than no form. `/contact-us/` never had a form to lose (see its own harvest file), so adding one would have been new functionality invented in a parity rebuild, not ported content. **What ships instead, on both routes:** a real `mailto:damianmasonoffice@gmail.com`, a real `tel:` on 888.304.0702, and the source's own "We typically respond within one business day" promise — which is a person answering, not a queue. `/contact-us/` also spells out the five lines to send, so the email carries what the dropped form's fields would have collected. **AMENDED: there is a backend now.** `app/api/contact/route.ts` shipped later and `/contact-us/` renders a real form against it. The reasoning above is unchanged and is exactly why that endpoint answers 501 and lets the form replace itself with the email address and phone number when no delivery provider is configured: a form that silently drops a meeting planner's date and budget is still strictly worse than no form. `/collaboration-opportunities/` deliberately still has none, and its field list and DOM order stay preserved verbatim in `_source/pages/collaboration-opportunities.md` section 7. Unrelated and still shipping: the Mailchimp newsletter `<form>` on `/contact-us/` and `/join-the-conversation/`, which posts to `damianmason.us13.list-manage.com` and works because Mailchimp is the backend. |
...
| The 16 macOS screenshots used as marketing imagery | Raw screen captures with default filenames (`Screenshot-2023-04-25-at-10.51.10-AM.png` and friends), several with browser chrome and player UI still visible, two with a raw U+202F narrow no-break space inside the filename. `_source/asset-map.json` holds 15 `Screenshot-*` sources plus `acres-tv-screenshot.png`, which is the same kind of capture under a tidier name. They are retained in `_source/media/` and mapped in `_source/asset-map.json` because several are the only artwork that exists for their subject, but every one is a quality ceiling. Real photography is **open item #4**. The 13 of them that the pipeline also transcodes into `public/img/photos/` have their own row below. |
| `keynote-stage-wide.jpg` (was on `/meeting-coordinators/`) and `keynote-stage-xtremeag.jpg` (was on `/`) | Two frames of the same moment on the XtremeAg stage, shot seconds apart and near identical: same jacket, same slide on the screen, same lighting rig. Damian is roughly 200px tall in a 1500 by 2000 frame in both, which is the wide-stage problem DESIGN_SYSTEM.md section 6 is built around. The tighter crop of the second, `keynote-stage-xtremeag-portrait.jpg`, is the one that ships, on `/xtreme-ag/`. Nothing in the two wide frames is absent from the crop. |
| `book-signing-xtremeag.png` (was on `/reviews/`) | Not an XtremeAg photograph at all: it is the StoneX Ag & Dairy Market Outlook book signing, the same 1512 by 1209 frame already shipping as `book-signing-stonex.jpg`. A duplicate under a wrong name. |
| `forbes-feature.png` (was on `/blog-news/`) | A promo card, not a photograph: a Forbes article headline set beside a portrait with a **READ ARTICLE >** button drawn into the pixels. A baked-in button that cannot be clicked is worse than no button, and DESIGN_SYSTEM.md section 6.4 rule 2 sends this slot to a `ruled` `Card` with the real headline and a real link instead. The article itself carries over as a link. |
| `speaking-collage.png` (was on `/`) | **This row was wrong, and the error cost the site two of its best photographs for seven phases.** It said the composite's other three panels were "wide room shots at a smaller scale than the standalone photographs that do ship", so nobody opened the file. Two of them are neither wide nor room shots. Panel 2 is a TIGHT three-quarter close-up, mid-sentence, hand raised, and it is tighter than any standalone photograph on this site. Panel 3 is a session shot from BEHIND a seated audience, which is the photography `docs/OPEN-ITEMS.md` item 4 asks the client to supply and records the site as not having. Both are now harvested by the `CROPS` table in `scripts/normalize-assets.mjs` into `speaking-closeup.png` and `tradeshow-floor-audience.png`, and placed on `/reviews/` and `/speaking/`. Only the assessment of panel 4 held up: it is the XtremeAg stage frame already shipping as `keynote-stage-xtremeag-portrait.jpg`. Panel 1 is a wider take on a stage frame the site already covers and is deliberately not harvested. The composite itself is deleted and recorded in `SKIPPED`. **The lesson is the row, not the file: a manifest entry that describes an asset instead of quoting a measurement is how a good frame gets written off, and nothing downstream re-checks it.** |
| The `$99/month` figure on `/boasg/` | The old hero set it as a second H1 directly above a "Join Today" button that went to the WooCommerce product page. The client checklist in `docs/build/STATE.md` reads "No Shop / Cart / Checkout / product pages; **no prices**; no 'Add to cart'", so the figure comes off with the commerce it was attached to, and the reasoning is also in the header comment of `app/boasg/page.tsx`: with no checkout behind it, a price is a number nobody can act on. The page still says the membership is monthly and closes on "There's no checkout. Tell Damian what you grow, sell, or finance, and he'll send you the terms and the next program date." Listed here because it is the one hard number from the old site that the new site does not carry anywhere, and the client may read their own checklist as "no purchase actions" rather than "no figures". If so the fee goes back as plain prose in the "What you get" block, which is not a purchase action. Pairs with **open item #1** on which BOASG button was meant. |
| Blog as a **top-level** nav item | Not dropped, moved: `/blog/` is now the third child of the Media dropdown in `content/site.ts`. On the old nav (`_source/pages/_global-chrome.md`, item 5) Blog sat at top level directly beside a Media parent whose only real child was `/blog-news/`, so two blog-ish destinations read as unrelated peers. Grouping them costs a reader one extra hover, and it is the **only** route on the site that got harder to reach: `/speaking/` and `/podcasts/` went from href-less parents to real hubs, and `/join-the-conversation/` went from nav-invisible to a footer link. Flagging it here because it is a change to the client's own navigation, not an oversight. |
| **Was "processed, not placed": the 13 screenshot-derived photographs.** That row is settled, in both directions. | The thirteen `public/img/photos/*.png` captures shipped in the bundle with no route referencing any of them, pending **open item #4**. The imagery pass answered it without waiting for the client, by reading DESIGN_SYSTEM.md 6.4 rule 3 rather than only rule 1: a capture may be evidence once the player chrome is cropped off, it sits on a plate with a hairline, and the cutline says what it is. Eight of the thirteen now do exactly that and are placed. **Placed:** `news-interview-cal-maine` and `cheddar-news-food-supply` (the two blog posts and the media list), `food-inflation-episode`, `cheddar-news-fertilizer-shortage` and `san-interview-2` (the media list), `equipment-factory-tour` and `xtremeag-video-interview` (`/xtreme-ag/`), and `field-day-panel` (the home page, from the earlier phase). All the chrome came off in the `CROPS` table in `scripts/normalize-assets.mjs`, which asserts an expected pixel size per rectangle so a drifting re-run fails instead of shipping a play button. **Deleted from `public/` and moved to `SKIPPED`:** `acres-tv-episode-grid`, `san-interview-1`, `xtremeag-cutting-the-curve-2`, plus `book-signing-xtremeag`, `keynote-stage-wide` and `keynote-stage-xtremeag` from the rows above; each is a duplicate of a frame that already ships or has no placement the design system permits, and the reason is on its `SKIPPED` entry. **Still processed and unplaced, 2 of the 13:** `green-screen-studio`, whose 1px edges came off in `CROPS` so placing it is a one-line change, and `xtremeag-cutting-the-curve-1`, which still carries its player chrome because nothing needed it. Crop it in `CROPS` before placing it. 8 placed plus 3 deleted plus 2 held is the whole thirteen. The originals in `_source/media/` are untouched in every case. |
| The retired 6-logo client wall in `public/img/logos/` | Kept on disk, referenced by no route. Merck and FCS of America are superseded by the supplied versions in `public/img/clients/`; John Deere, BASF, Helena and IPPA are the row above, parked on **open item #2**. They stay processed so that answering that item is a one-line change rather than a re-run of the asset pipeline. `acres-tv.png` sits in the same folder and is superseded by `photos/acres-tv-lockup.png`. |
...
**AMENDED 2026-08-07, and this is the sentence the whole media-kit rescue contradicts.** The Media Kit ZIP was never mirrored, and because it was never mirrored nobody could say what was in it, and because nobody could say what was in it the loss stayed invisible for seven phases. It held nine professional photographs. A second archive nobody knew about held eleven more, and the AV requirements document was in the same library. All of it is now at `_source/media-kit/`, inventoried with source URLs and SHA-256s in `PROVENANCE.md`. **The lesson is in the method, not the file:** this mirror was built by crawling 29 pages, so anything in the library that no page linked was invisible to it, and `_source/manifest.json` recorded that gap under "MEDIA ASSET MIRROR GAPS" and nobody followed it up. The link is still removed per client instruction; the assets are not. Several WordPress -1080x675 / -1024x577 / -300x300 derivatives referenced by srcset and og:image were not mirrored either - only the full-size originals were. Neither blocks the rebuild since the new site regenerates its own derivatives.
