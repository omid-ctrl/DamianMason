...
| `speaking-collage.png` (was on `/`) | A four-up collage composited on a black ground, named as a screenshot in DESIGN_SYSTEM.md section 6.4 and barred from a hero or a band by rule 1. Its fourth panel is the XtremeAg stage frame already shipping as `keynote-stage-xtremeag-portrait.jpg`, and the other three are wide room shots at a smaller scale than the standalone photographs that do ship. |
...
| **Processed, not placed:** the 13 screenshot-derived photographs in `public/img/photos/` — `acres-tv-episode-grid`, `cheddar-news-fertilizer-shortage`, `cheddar-news-food-supply`, `equipment-factory-tour`, `field-day-panel`, `food-inflation-episode`, `green-screen-studio`, `news-interview-cal-maine`, `san-interview-1`, `san-interview-2`, `xtremeag-cutting-the-curve-1`, `xtremeag-cutting-the-curve-2`, `xtremeag-video-interview` (all `.png`) | These are the macOS captures from the row above, and this row records what actually happens to them rather than only where the originals live. Twelve derive from a `_source/media/Screenshot-*.png` and one from `_source/media/acres-tv-screenshot.png`; each has an `out` entry in `_source/asset-map.json`, so `scripts/normalize-assets.mjs` transcodes all 13 into `public/img/photos/` and they ship in the deployed bundle at 8.0 MB. **No route references any of them,** and that is deliberate: DESIGN_SYSTEM.md section 6.4 bars a raw screen capture from a hero or a band, and every slot these were pulled for now carries either a real photograph or the typographic treatment rule 2 calls for. They stay processed for the same reason the retired logo wall does — if **open item #4** comes back as "no new photography, use what exists", placing one is a one-line change rather than a re-run of the pipeline. If instead the answer is that unplaced art must not ride in the bundle, delete these 13 from `public/img/photos/` and drop their `out` entries from `asset-map.json`; the originals in `_source/media/` are untouched either way. Distinct from the five rows above, which name photographs that were on a specific old page and were replaced for a specific reason. |
